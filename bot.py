...
async def get_track_entries(track: str):
    entries = await times_col.find({"track": track}, {"_id": 0}).to_list(None)
    entries.sort(key=lambda x: time_to_seconds(x["time"]))
    return entries

async def get_all_tracks():
    return await times_col.distinct("track")

async def get_all_data():
    data = {}
    for track in await get_all_tracks():
        data[track] = await get_track_entries(track)
    return data
...
async def on_reaction_add(reaction, user):
    ...
    if str(reaction.emoji) == "✅":
        ...
        existing = await times_col.find_one({"track": track, "uid": uid})
        if existing:
            ...
            if new_seconds >= old_seconds:
                ...
                return
            await times_col.delete_one({"track": track, "uid": uid})
...
async def remove_track(ctx, *, track_name: str):
    ...
    result     = await times_col.delete_many({"track": track_name})
...
async def remove_time(ctx, member: discord.Member, *, track_name: str):
    track_name = track_name.title()
    result     = await times_col.delete_one({"track": track_name, "uid": member.id})
    if result.deleted_count == 0:
